...
function createNode() {
  var div = document.createElement('div');
  var smile = twemoji.parse('☺', { size: 36 });
  var node = '<button class="btn icon-btn PollCreator-btn js-tooltip js-twemoji" type="button" title="絵文字を追加" data-delay="150" tabindex="0">' + '<span class="Icon Icon--poll">' + smile + '</span>' + '<span class="text">絵文字</span>' + '</button>';
  div.classList.add('TwemojiCreater');
  div.innerHTML = node;
  return div;
}

_reactDom2['default'].render(_react2['default'].createElement('div', { className: 'foobar' }), document.getElementsByTagName('body'));
